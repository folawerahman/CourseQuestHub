...
router.post('/', async (req, res) => {
    ...
    const resetLink = `${clientURL}/forgotPassword?token=${resetToken}&id=${user._id}`;

    // Use the sendEmail function to send the email, pass user.firstname and resetLink as parameters
    const emailSent = await sendEmail(user.email, "Course Quest Hub: Forgot Password Reset Request", generateResetEmail(user.firstname, resetLink));
...
function generateResetEmail(firstname, resetLink) {
    ...
    <p>If you did not request a forgot password reset, please ignore this email.</p>
`;
}
